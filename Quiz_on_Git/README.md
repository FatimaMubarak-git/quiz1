...
- [ ] git status
- [ ] git add .
- [T ] git commit -m "my_file.txt is added"
- [ ] git add my_file.txt
...
- [ T] execute the following commands:
...
- [ T] Create a new branch named ``debugging`` and ``HEAD`` will stay pointing at ``master``   
- [ ] None of the above
...
- [ T] After ``git init`` and the first commit C<sub>1</sub>:
...
No , since  we can not see *.txt since it is in .gitignore is hidden . 
Question 8
----------
...
``git checkout v2.0``
The head will be on b since its tagged by v2.0
Question 9:
----------
...
Head will be on c4 since when we make git checkout experiment the head will be on c4 . 
Master will be on c4. 
Question 10:
-----------
...
No , since the new file is not modified 
.
